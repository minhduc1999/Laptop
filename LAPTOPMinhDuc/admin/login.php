	...
	<?php
					    if (isset($_POST['login'])) {
						    if(empty($_POST['username']) or empty($_POST['password'])){
						        ...
						        $sql ="select * from tbl_nhanvien where taiKhoan ='$username' and matKhau ='$pass' ";
						        ...
						        if ($num == 0) {
						        	...
						        	$rs = mysqli_fetch_array($query) or die("Lỗi truy vấn");
						        		if ($rs['quyen']==1) {
						        			$_SESSION['username'] = $username;
						        			echo "<script> alert('Đăng nhập thành công!')</script>";
          									echo "<script> window.location = '../admin/quan_tri.php';</script>";
						        		}
						        		else{
						        			$_SESSION['username'] = $username;
						        			echo "<script> alert('Đăng nhập thành công!')</script>";
         									echo "<script> window.location = '../admin/nhanvien.php';</script>";
						        		}
						        }
						    }
						}      
